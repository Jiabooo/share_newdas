...
def main():
    ...
    model = CSRNet1()
    pretrained=torch.load(r"/home/ch/csrnet_mask_A_1e7/0\6ceng_mask_63.0_102.5.tar")
    model = model#.cuda()
...
def test(test_list, model,mask_model):
    ...
    for i, (img, target,mask_target) in enumerate(test_loader):
        ...
        with torch.no_grad():
            output = model(img)
    t2 = time.time()
    print((t2-t1)/len(test_loader))
    
    for i, (img_path,img, target,mask_target) in enumerate(test_loader):
        img1 = img_path[0]
        ...
        img = Variable(img)
        with torch.no_grad():
            output,mask = mask_model(img)
            mask = torch.where(mask>0.1,1,0)
            output,mask = model(img,mask)
            ...
            l = transform(l)
            output,mask = model(l.unsqueeze(0).cuda(),mask1)
