...
def train(train_list, model, criterion,count_criterion, mask_criterion,optimizer, epoch,model1):
    ...
    for i,(img, target,count_target, mask_target,depth)in enumerate(train_loader):
        ...
        loss_c = count_criterion(output, count_target) * 0.4
        ...
        if i % args.print_freq == args.print_freq-1:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Loss_locate {loss_c.val:.4f} ({loss_c.avg:.4f})\t'
                  .format(
                   epoch, i, len(train_loader), batch_time=batch_time,
                   data_time=data_time, loss=losses, loss_d=losses_d, mask_loss=losses_mask, loss_c = losses_c))
