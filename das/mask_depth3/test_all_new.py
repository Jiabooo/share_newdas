...
def main():
    ...
    model = CSRNet()
    pretrained = torch.load(r"G:\renqun\das\das\mask_depth\mask_depth.tar") # 最后那个best模型的路径
    model = model.cuda()
    model.load_state_dict(pretrained['state_dict'])

    mask_model = CSRNet1()
    pretrained = torch.load(r"G:\renqun\das\das\mask_depth\mask_depth.tar") # second_A路径，用于掩膜
